...
def build_node_props(record: dict) -> dict:
    ...
    name = record.get("Official English Name") or record.get("Romanized Name") or record.get("source_name")
    ...
    with driver.session() as session:
        ...
        for i, record in enumerate(records):
            try:
                ...
                session.run(MERGE_QUERY, opwikiID=props_clean["opwikiID"], props=props_clean)
                loaded += 1
            except Exception as e:
                failed.append({"source_name": record.get("source_name", "?"), "error": str(e)})
