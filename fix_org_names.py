...
FIXES = {
    ...
    "Beautiful PiratesStraw Hat Grand Fleet":
        ["Beautiful Pirates", "Straw Hat Grand Fleet"],
    "Inuarashi Musketeer SquadNinja-Pirate-Mink-Samurai Alliance":
        ["Inuarashi Musketeer Squad", "Ninja-Pirate-Mink-Samurai Alliance"],
    "Kid PiratesNinja-Pirate-Mink-Samurai Alliance":
        ["Kid Pirates", "Ninja-Pirate-Mink-Samurai Alliance"],
    "Tonta CorpsStraw Hat Grand Fleet":
        ["Tonta Corps", "Straw Hat Grand Fleet"],
    "Tontatta KingdomScouting Unit":
        ["Tontatta Kingdom", "Scouting Unit"],
    "Tontatta KingdomScouting UnitStraw Hat Grand Fleet":
        ["Tontatta Kingdom", "Scouting Unit", "Straw Hat Grand Fleet"],
    "Tontatta KingdomTontatta PiratesStraw Hat Grand Fleet":
        ["Tontatta Kingdom", "Tontatta Pirates", "Straw Hat Grand Fleet"],
    "Tontatta PiratesStraw Hat Grand Fleet":
        ["Tontatta Pirates", "Straw Hat Grand Fleet"],
    "Tontatta TribeScouting Unit":
        ["Tontatta Tribe", "Scouting Unit"],
...
def fix_org(session, old_name: str, target_names: list[str], log: list) -> int:
    ...
    if not affiliations:
        log.append({"action": "skip", "old": old_name, "reason": "no characters linked"})
        return 0
    ...
    for target_name in target_names:
        ...
        """, org_id=target_id, name=target_name)
    ...
    with driver.session() as session:
        for old_name, target_names in FIXES.items():
            ...
            if rels > 0:
                total_rels += rels
                total_nodes_fixed += 1
                action = "renamed" if len(target_names) == 1 else f"split into {len(target_names)}"
