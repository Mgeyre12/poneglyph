...
def run_import(driver, arcs: list[dict]) -> None:
    ...
    with driver.session() as session:
        ...
        result = session.run("MATCH (ch:Chapter) RETURN ch.number AS number ORDER BY ch.number")
        ...
        for number in chapter_numbers:
            ...
            if arc is None:
                skipped.append({"chapter": number, "reason": "no arc range covers this chapter"})
                continue
