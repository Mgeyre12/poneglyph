...
def collect_rel_schema(session) -> list[dict]:
    ...
    types_result = session.run("CALL db.relationshipTypes() YIELD relationshipType RETURN relationshipType ORDER BY relationshipType")
    ...
    for rel_type in rel_types:
        ...
        for (from_label, to_label), props in sorted(seen.items()):
            rels.append({"type": rel_type, "from": from_label, "to": to_label, "props": props})
...
def build_markdown(label_schema: dict, rels: list[dict], counts: dict) -> str:
    ...
    for label, props in sorted(label_schema.items()):
        prop_str = ", ".join(
            f"`{k}`: {t}" for k, t in sorted(props.items())
        )
    ...
    for rel in rels:
        ...
        lines.append(
            f"- `(:{rel['from']})-[:{rel['type']}{prop_str}]->(:{rel['to']})`"
        )
