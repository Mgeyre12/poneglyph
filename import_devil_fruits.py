...
LOG_FILE  = os.path.join(os.path.dirname(__file__), "..", "logs", "fruit_user_skipped.log")
...
def build_fruit_props(record: dict) -> dict:
    ...
    raw_name = record.get("Official English Name") or record.get("Romanized Name") or record["source_name"]
    ...
    props = {
        ...
        "debut_chapter": parse_debut_chapter(record.get("Usage Debut") or record.get("Fruit Debut")),
    }
    ...
    with driver.session() as session:
        ...
        for i, record in enumerate(fruits):
            ...
            try:
                ...
                skipped.append({"fruit_id": fruit_id, "reason": f"node write failed: {e}"})
                continue

            rels_written += process_user_field(
                record.get("Current User"), "current", fruit_id, lookup, session, skipped
            )
            rels_written += process_user_field(
                record.get("Previous User"), "former", fruit_id, lookup, session, skipped
            )
...
def main():
    ...
    char_file = os.path.join(os.path.dirname(__file__), "data", "full-character-data-processed-2.json")
